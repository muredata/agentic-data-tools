# agentic-data-tools

External tools to interact with data platforms and tooling:

## Data stack

### Microsoft Fabric

- Skills: [microsoft/skills-for-fabric](https://github.com/microsoft/skills-for-fabric)
- MCP: [microsoft/mcp/servers/Fabric.Mcp.Server](https://github.com/microsoft/mcp/tree/main/servers/Fabric.Mcp.Server)
- CLI: [microsoft/fabric-cli](https://github.com/microsoft/fabric-cli)

**/ Community**
- [data-goblin/power-bi-agentic-development](https://github.com/data-goblin/power-bi-agentic-development)
- [maxanatsko/pbir.tools](https://github.com/maxanatsko/pbir.tools)
- [RuiRomano/powerbi-agentic-mcp-cloud-agent](https://github.com/RuiRomano/powerbi-agentic-mcp-cloud-agent)
- [RuiRomano/powerbi-agentic-plugins](https://github.com/RuiRomano/powerbi-agentic-plugins)
- [tmdaidevs/ontology-mcp-server](https://github.com/tmdaidevs/ontology-mcp-server)
- [tmdaidevs/powerbi-creator-skill](https://github.com/tmdaidevs/powerbi-creator-skill)

### Databricks

- Skills: [databricks/databricks-agent-skills](https://github.com/databricks/databricks-agent-skills)
- MCP: [databrickslabs/mcp](https://github.com/databrickslabs/mcp)
- CLI: [databricks/cli](https://github.com/databricks/cli)

**/ Others**
- AI Dev Kit: [databricks-solutions/ai-dev-kit](https://github.com/databricks-solutions/ai-dev-kit)
- App templates: [databricks/app-templates](https://github.com/databricks/app-templates)
- MLflow: [mlflow/skills](https://github.com/mlflow/skills)

**/ Community**
- TODO

### Others

- Airflow: [astronomer/agents](https://github.com/astronomer/agents)
- Clickhouse: [clickhouse/agent-skills](https://github.com/ClickHouse/agent-skills)
- Confluent: [confluentinc/agent-skills](https://github.com/confluentinc/agent-skills)
- DataFusion: [datafusion-contrib/datafusion-skills](https://github.com/datafusion-contrib/datafusion-skills)
- DataFusion Comet: [ddatafusion-comet/skills](https://github.com/apache/datafusion-comet/tree/main/.claude/skills)
- dbt: [dbt-labs/dbt-agent-skills](https://github.com/dbt-labs/dbt-agent-skills)
- duckdb: [duckdb/duckdb-skills](https://github.com/duckdb/duckdb-skills)
- Google: [google/skills](https://github.com/google/skills)
- Google databases: [googleapis/mcp-toolbox](https://github.com/googleapis/mcp-toolbox)
- Hugging Face: [huggingface/skills](https://github.com/huggingface/skills)
- Motherduck: [motherduckdb/agent-skills](https://github.com/motherduckdb/agent-skills/)
- Motherduck: [motherduckdb/mcp-server-motherduck](https://github.com/motherduckdb/mcp-server-motherduck)
- Snowflake: [snowflake-labs/snowflake-ai-kit](https://github.com/Snowflake-Labs/snowflake-ai-kit)
- Spark: [kubeflow/mcp-apache-spark-history-server](https://github.com/kubeflow/mcp-apache-spark-history-server)
- Spark: [yaooqinn/spark-history-cli](https://github.com/yaooqinn/spark-history-cli)
- Terraform: [antonbabenko/terraform-skill](https://github.com/antonbabenko/terraform-skill)

## Cloud providers

### Azure

- Skills: [microsoft/azure-skills](https://github.com/microsoft/azure-skills/) 
- MCP: [microsoft/azure-skills](https://github.com/microsoft/azure-skills/) (includes Azure and Foundry MCP)
- CLI: [azure/azure-cli](https://github.com/Azure/azure-cli)

### AWS

- Skills: [agent-toolkit-for-aws/skills](https://github.com/aws/agent-toolkit-for-aws/tree/main/skills) 
- MCP: [awslabs/mcp](https://github.com/awslabs/mcp)
- CLI: [aws/aws-cli](https://github.com/aws/aws-cli)

**/ Others**
- Agent Toolkit: [agent-toolkit-for-aws](https://github.com/aws/agent-toolkit-for-aws)

### Google Cloud

- Skills: [google/skills](https://github.com/google/skills) 
- MCP: [googleapis/gcloud-mcp](https://github.com/googleapis/gcloud-mcp)
- CLI (docker): [googlecloudplatform/cloud-sdk-docker](https://github.com/GoogleCloudPlatform/cloud-sdk-docker)

--

- Google Workspace CLI: [googleworkspace/cli](https://github.com/googleworkspace/cli)